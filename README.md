...
Dessa forma, é necessário fazer algumas coisas mais para a instalar no Fedora 43+ e, acredito eu, nas demais distribuições RHEL. 
 ...
 - A outra é adaptar o setup em Docker para Podman, este é preferível já que assim são evitados diversos problemas de compatibilidade e ambiente virtual:
    - Esse setup também é bem mais simples, basta _clonar_ este repositório:
      ```
    ...
    - Agora, você pode executa-la:
      ...
      ```
    - Ou executar o nosso _pod_ em _kubernete_:
      ```
      xhost +local:
      (edite o arquivo ardupilot-sitl/ardupilot_sitl.yaml para que a variavel de ambiente XAUTHORITHY seja a mesma no container e no _host_)
...
Para fins de controle externo, as portas seriais do drone são traduzidas para TCP/IP nas portas, 5760, 5762 e 5763.
...
Dessa forma, é necessário fazer algumas coisas mais para a instalar no Fedora 43+ e, acredito eu, nas demais distribuições RHEL. 
 - Uma das possibilidades é utilizar um script de instalação reescrito para essas distribuições - para este, infelizmente, não temos uma solução pronta.
 - A outra é adaptar a instalação dentro de um container em Podman, essa é mais complexa pois é necessário configurar o _host_ para a passagem da _GPU_ para o contêiner.
    - Então, é necessário instalar os prérequisitos nescessários para sua placa de vídeo:
       - Placas de Vídeo AMD (Veja [1](https://access.redhat.com/solutions/7073764) e [2](https://instinct.docs.amd.com/projects/container-toolkit/en/latest/container-runtime/cdi-guide.html)):
          - Adicione o repositório Codeready builder:
         ```
         ...
         ```
    - Depois de instalar os prérequisitos, basta _clonar_ este repositório:
   ```
   git clone https://github.com/Falcon-IFSP/drone-sim.git
   cd drone-sim
   ```
   ...
   ```
   xhost +local:
   (edite o arquivo gazebo-harmonic/amd/gazebo_harmonic.yaml para que a variavel de ambiente XAUTHORITHY seja a mesma no container e no _host_)
